...
object dataframe_two extends App{
    override def main(args: Array[String]){
        ...
        seq_df.show()
        //method 2
        val csv_df1 = spark.read.format("csv").option("header",true).load("src\\main\\scala\\GadaiteGroupID\\DataSets\\iris-data.csv")
        csv_df1.show()
        val csv_df2 = spark.read.format("csv").option("header",false).load("src\\main\\scala\\GadaiteGroupID\\DataSets\\iris-data.csv")
        csv_df2.show()
        val csv_df3 = spark.read.format("csv").load("src\\main\\scala\\GadaiteGroupID\\DataSets\\iris-data.csv")
        csv_df3.show()
